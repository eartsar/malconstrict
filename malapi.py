...
def raw_get_anime_details(id, mine=0, auth_token=None):
    ...
    if mine == 1:
        ...
        if len(auth_token) != 2:
            ...
            return response
        response = requests.get(apiurl + '/anime/' + str(id) + '?mine=' + str(mine), auth_token).content
    else:
        response = requests.get(apiurl + '/anime/' + str(id) + '?mine=' + str(mine)).content
    return response
